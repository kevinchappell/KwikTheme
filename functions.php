<?php
/**
 * OpenPower functions and definitions.
 *
...
function op_setup() {
	/*
	 * Makes OpenPower available for translation.
	 *
	 * Translations can be added to the /languages/ directory.
	 * If you're building a theme based on OpenPower, use a find and replace
...
function number_to_class($num = 1, $echo = true){
	$class = '';
	if($num == 1){ return;}
	else if($num == 2){$class = 'halves'; }
	else if($num == 3){$class = 'thirds';}
	else if($num == 4){$class = 'fourths';}
	else if($num == 5){$class = 'fifths';}
...
function op_child_links(){
	...
	if(is_page() || is_home()){
		...
		if(is_home()){
			...
			if ($root==0) $child_links = wp_list_pages("sort_column=menu_order&title_li=&child_of=". $post->post_parent ."&echo=0&depth=1");
		} else {			
			$child_links = wp_list_pages("sort_column=menu_order&title_li=&child_of=". $post->ID ."&echo=0&depth=1");	
		}
