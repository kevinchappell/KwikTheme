 ...
 */
 ?>
    ...
    <div class="text_wrap<?php has_client_link($post->ID); ?>">
    <?php content_client_link($post->ID); ?>
		<header class="entry-header">
        <h3 class="entry-title">
          <a href="<?php the_permalink(); ?>" title="<?php echo esc_attr( sprintf( __( 'Permalink to %s', 'op' ), the_title_attribute( 'echo=0' ) ) ); ?>" rel="bookmark">
            <?php the_title(); ?>
          </a>
		...
		<?php
			...
			$cat_info .= '</a>';
        ?>
        <?php if(get_query_var('taxonomy') !== 'technical_resources_category') { ?>
			<span class="entry-date"><?php echo human_time_diff( get_the_time('U'), current_time('timestamp') ).' ago'; ?>&nbsp; &#183; &nbsp;</span>
		<?php } ?>
		<?php echo $cat_info; ?>
    <?php if($source){ ?>&nbsp; &#183; &nbsp;<span class="entry-source">
    <?php _e('Source:', 'kwik');?> <?php echo $source_link ? '<a href="'.$source_link.'" target="_blank">' : ''; ?><?php echo $source; ?><?php echo $source_link ? '</a>' : ''; ?></span><?php } ?>
