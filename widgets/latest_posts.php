...
class Latest_Posts_Widget extends WP_Widget {
	...
	function Latest_Posts_Widget() {
		...
		/* Create the widget. */
		$this->WP_Widget( 'latest-posts-widget', esc_html__('Latest Posts', 'kwik'), $widget_ops, $control_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		$num_posts_query = new WP_Query( "{$cats_to_include}showposts={$num_posts}&post_type={$post_type}&offset={$post_offset}" );
...
if ( has_post_thumbnail() && $show_thumbs != false ) { 
  ...
  ?>
    ...
    <?php if($excerpt_length != 0) echo '<div class="entry-summary">'.neat_trim(get_the_excerpt(), $excerpt_length, $delim, $neat).'</div>';  ?>
...
<?php		endif;
	...
	function form( $instance ) {
		...
		$defaults = array( 'title' => esc_html__('Latest Items', 'kwik'), 
		'category_id' => '', 
		'num_posts' => 3, 
		'post_offset' => 0, 
		'post_type' => 'post',
