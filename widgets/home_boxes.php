 ...
 * @since 0.1
 */
add_action('widgets_init', 'home_boxes_load_widget');
 ...
 * @since 0.1
 */
function home_boxes_load_widget()
{
...
function kt_more_news_scroll($offset)
{
    ...
    if (!empty($widget_options['home_box_content'])) {
        $news_scroll_args['post__not_in'] = $widget_options['home_box_content'];
    }
...
jQuery.loadScript = function (url, arg1, arg2) {
  ...
  if (load){
    ...
    $js .= "function set_container_width() { var totalContent = 0; $('#news_scroll .hentry').each(function () { pixel_width = parseInt($(this).innerWidth()); pixel_margin = parseInt($(this).css('margin-right').replace('px', '')); pixel_total = pixel_width + pixel_margin; $(this).width(pixel_width).css('margin-right', pixel_margin + 'px'); totalContent += pixel_total; }); $('#main #news_scroll .overview').css('width', totalContent);}";

    $js .= "function set_container_height() {    $('#main #news_scroll .viewport').css('height', $('#news_scroll .hentry').height());}set_container_width();set_container_height();var offset = parseInt($('#news_scroll_offset').text());var clicks = 0;";
...
class Home_Boxes extends WP_Widget
{
    ...
    public function widget($args, $instance)
    {
        ...
        if (!empty($home_boxes[0]) || !empty($home_boxes[0])) {
            $news_scroll_args['post__in'] = $instance['home_box_content'];
        }
...
echo esc_attr($title);?>" /></p>
        ...
        <p><label for="<?php
echo $this->get_field_id('fallback');?>"><?php
_e('Fallback:');?></label>
<?php
echo $fallback;?>
        </p>
...
<?php
        ...
        ?>
                ...
                <div id="add_remove_wrap"><span id="add_box" class="button-secondary" title="Add Box">+</span><?php
if (count($instance['home_box_content']) > 1) {
            echo '<span id="remove_box" class="button-secondary" title="Remove Box">-</span>';
        }
        ?></div>
              </div>
