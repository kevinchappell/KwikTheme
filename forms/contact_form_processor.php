...
function send_email($post, $files) {
    ...
    if ($post['y_phone'] != '') {
        $message .= '<tr><td width="33%">Phone:</td><td>' . $post['y_phone'] . '</td></tr>';
    }
    ...
    if (!empty($files['y_file']['name'])) {
        ...
        if (!is_writable($upload_path)) {
            $validation[] = array("field" => "mail_result", "msg" => "You cannot upload to the specified directory, please CHMOD it to 777.");
        }
